...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        hl2 = 4 / 10  # 监测区域高度距离图片顶部比例
        wl2 = 3.5 / 10  # 监测区域高度距离图片左部比例
        hl3 = 1  # 监测区域高度距离图片顶部比例
        wl3 = 2.1/10  # 监测区域高度距离图片左部比例
        hl4 = 9/10  # 监测区域高度距离图片顶部比例
        wl4 = -2.2 / 10  # 监测区域高度距离图片左部比例
        if webcam:
            for b in range(0, img.shape[0]):
                ...
                pts = np.array([[int(img[b].shape[2] * (wl1-0.08)), int(img[b].shape[1] * (hl1-0.03))],  # pts1
                                [int(img[b].shape[2] * (wl2+0.08)), int(img[b].shape[1] * (hl2-0.03))],  # pts2
                                [int(img[b].shape[2] * (wl3+0.08)), int(img[b].shape[1] * (hl3+0.04))],  # pts3
                                [int(img[b].shape[2] * (wl4-0.08)), int(img[b].shape[1] * (hl4+0.04))]], np.int32)
            ...
            pts = np.array([[int(img.shape[2] * (wl1 - 0.08)), int(img.shape[1] * (hl1-0.03))],  # pts1
                            [int(img.shape[2] * (wl2 + 0.08)), int(img.shape[1] * (hl2-0.03))],  # pts2
                            [int(img.shape[2] * (wl3 + 0.08)), int(img.shape[1] * (hl3+0.04))],  # pts3
                            [int(img.shape[2] * (wl4 - 0.08)), int(img.shape[1] * (hl4+0.04))]], np.int32)
            mask = cv2.fillPoly(mask, [pts], (255, 255, 255))
        ...
        for i, det in enumerate(pred):  # detections per image
            if webcam:  # batch_size >= 1
                ...
                mask = cv2.fillPoly(zeros, [pts], color=(168, 165, 55))
                im0 = cv2.addWeighted(im0, 1, mask, 0.3, 0)
                cv2.polylines(im0, [pts], True, (255, 255, 0), 2)
            else:
                p, s, im0 = path, '', im0s
                cv2.putText(im0, "Detection_Region", (int(im0.shape[1] * wl1 - 5), int(im0.shape[0] * hl1 - 5)),
                            cv2.FONT_HERSHEY_SIMPLEX,
